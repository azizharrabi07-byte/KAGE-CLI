...
يمكنك إضافة أدوات خارجية إلى KAGE باستخدام _Model Context Protocol_، أو MCP. يدعم KAGE خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات KAGE](https://kage.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/kage` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى KAGE تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم KAGE بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك KAGE المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `kage mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن KAGE الرموز بشكل آمن في `~/.local/share/kage/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل KAGE إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات KAGE مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط KAGE بحساب Sentry الخاص بك.
