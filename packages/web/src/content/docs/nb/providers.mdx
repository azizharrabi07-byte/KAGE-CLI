---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i KAGE.
---

import config from "../../../../config.mjs"
export const console = config.console

KAGE bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i KAGE-konfigurasjonen.
...
i `~/.local/share/kage/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i KAGE
konfig.
...
KAGE Zen er en liste over modeller levert av KAGE-teamet som har vært
testet og verifisert for å fungere godt med KAGE. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med KAGE Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `KAGE Zen` og gå til [kage.ai/auth](https://kage.ai/zen).
...
Det fungerer som alle andre leverandører i KAGE og er helt valgfritt å bruke.

---

## KAGE Go

KAGE Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av KAGE-teamet som har vært
testet og verifisert for å fungere godt med KAGE.

1. Kjør kommandoen `/connect` i TUI, velg `KAGE Go`, og gå til [kage.ai/auth](https://kage.ai/zen).
...
Slik bruker du Amazon Bedrock med KAGE:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører KAGE:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `kage.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i kage støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere kage til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at KAGE skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom KAGE-konfigurasjonen.
...
for å lagre token i KAGE auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
KAGE bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse KAGE
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`kage.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `kage.json`:
...
Slik bruker du GitHub Copilot-abonnementet med KAGE:
...
Slik bruker du Google Vertex AI med KAGE:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom KAGE, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session) plugin for automatisk å logge hver KAGE-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver KAGE-samtale oppført som en separat økt.
...
Du kan konfigurere KAGE for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere KAGE for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere KAGE for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for KAGE. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/kage) for detaljer.
:::
...
Slik bruker du Ollama Cloud med KAGE:
...
3. Kopier API-nøkkelen for bruk i KAGE.
...
6. **Viktig**: Før du bruker skymodeller i KAGE, må du hente modellinformasjonen lokalt:
...
KAGE Zen er en liste over testede og verifiserte modeller levert av KAGE-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>KAGE Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **KAGE Zen**.

   ```txt
   /connect
   ```

3. Skriv inn KAGE API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom KAGE-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom KAGE-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med KAGE:
...
Du kan også tilpasse modeller gjennom KAGE-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med KAGE. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `kage.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar KAGE forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `kage auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk KAGE-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i KAGE-konfigurasjonen.
