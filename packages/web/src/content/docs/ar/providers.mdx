---
title: المزوّدون
description: استخدام أي موفّر LLM في KAGE.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم KAGE ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات KAGE.
...
في `~/.local/share/kage/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات KAGE.
...
KAGE Zen هي قائمة نماذج يوفّرها فريق KAGE وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع KAGE. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع KAGE Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر kage، ثم انتقل إلى [kage.ai/auth](https://kage.ai/auth).
...
يعمل مثل أي مزوّد آخر في KAGE واستخدامه اختياري بالكامل.

---

## KAGE Go

KAGE Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق KAGE، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع KAGE.

1. شغّل الأمر `/connect` في TUI، واختر `KAGE Go`، ثم انتقل إلى [kage.ai/auth](https://kage.ai/zen).
...
لاستخدام Amazon Bedrock مع KAGE:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل kage:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `kage.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل KAGE غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين kage لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل kage بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات kage.
...
تخزين الرمز في مساحة تخزين مصادقة kage.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم KAGE نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر KAGE
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`kage.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع kage:
...
لاستخدام Google Vertex AI مع KAGE:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر kage، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session) لتسجيل كل محادثة KAGE تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة KAGE مدرجة كجلسة مستقلة.
...
يمكنك ضبط kage لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط kage لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط kage لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع KAGE. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/kage) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في KAGE.
...
6. **مهم**: قبل استخدام نماذج السحابة في KAGE، يجب سحب معلومات النموذج محليا:
...
KAGE Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق KAGE. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>KAGE Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **KAGE Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ KAGE.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات kage.
...
5. يمكنك أيضا تخصيصها عبر إعدادات kage. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات kage. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع kage. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `kage.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ KAGE بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `kage auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات kage وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات kage.
