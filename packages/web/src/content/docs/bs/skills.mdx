...
Agent skills omogucavaju KAGE da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
KAGE pretrazuje ove lokacije:

- Konfiguracija projekta: `.kage/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/kage/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, KAGE ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.kage/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.kage/skills/git-release/SKILL.md` ovako:
...
KAGE navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `kage.json`:
...
**Za ugradene agente** (u `kage.json`):
