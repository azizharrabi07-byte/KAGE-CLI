...
Con KAGE puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, KAGE usa nuestro propio tema `kage`.
...
KAGE viene con varios temas integrados.
...
- Quiere que KAGE coincida con la apariencia de su terminal
...
KAGE admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/kage/themes/*.json` o `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.kage/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.kage/themes/*.json`
