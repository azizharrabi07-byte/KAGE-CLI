...
# 套件管理員
npm i -g kage-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install kage             # Windows
choco install kage             # Windows
brew install anomalyco/tap/kage # macOS 與 Linux（推薦，始終保持最新）
brew install kage              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S kage            # Arch Linux (Stable)
paru -S kage-bin               # Arch Linux (Latest from AUR)
mise use -g kage               # 任何作業系統
nix run nixpkgs#kage           # 或使用 github:anomalyco/kage 以取得最新開發分支
```
...
KAGE 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/anomalyco/kage/releases) 或 [kage.ai/download](https://kage.ai/download) 下載。
...
KAGE 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，KAGE 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://kage.ai/docs/agents) 的資訊。

### 線上文件

關於如何設定 KAGE 的詳細資訊，請參閱我們的 [**官方文件**](https://kage.ai/docs)。

### 參與貢獻

如果您有興趣參與 KAGE 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 KAGE 進行開發

如果您正在開發與 KAGE 相關的專案，並在名稱中使用了 "kage"（例如 "kage-dashboard" 或 "kage-mobile"），請在您的 README 中加入聲明，說明該專案並非由 KAGE 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/kage)
