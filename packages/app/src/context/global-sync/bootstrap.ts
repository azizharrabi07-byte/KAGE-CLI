...
import { getFilename } from "@kage/core/util/path"
import { retry } from "@kage/core/util/retry"
...
import { NormalizedProviderListResponse } from "@kage/session-ui/context"
    ...
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("kage-test"))
            .slice()
