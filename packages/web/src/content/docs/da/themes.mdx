...
Med KAGE kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger KAGE vårt eget `kage`-tema.
...
KAGE støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/kage/themes/*.json` eller `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.kage/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.kage/themes/*.json`
