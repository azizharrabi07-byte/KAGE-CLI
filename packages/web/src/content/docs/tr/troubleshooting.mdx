...
kage ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\kage\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `kage --log-level DEBUG`.

---

## Depolama

kage, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/kage/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\kage` yapistirin
...
kage Desktop arka planda yerel bir kage sunucusu (`kage-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `kage` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/kage/kage.jsonc` (veya `~/.config/kage/kage.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/kage/kage.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\kage\kage.jsonc` yapistirin
...
kage diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/kage/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\kage\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), kage'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. kage Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/kage`
- **Linux**: `~/.cache/kage` dizinini silin (veya `rm -rf ~/.cache/kage` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\kage` yapistirin

3. kage Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

kage Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`kage.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta kage Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, kage ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

kage Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda kage icin bildirimler etkinse
...
1. kage Desktop'u kapatin
2. Su dosyalari bulun ve silin (kage Desktop uygulama veri dizininde yer alirlar):

- `kage.settings.dat` (desktop varsayilan sunucu URL'si)
- `kage.global.dat` ve `kage.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
kage ile ilgili bir sorun yasiyorsaniz:
...
3. `kage upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `kage models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\kage`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. kage, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\kage`

2. En guncel provider paketlerini yeniden kurmak icin kage'u yeniden baslatin

Bu, kage'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
kage, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
