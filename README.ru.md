...
# Менеджеры пакетов
npm i -g kage-ai@latest        # или bun/pnpm/yarn
scoop install kage             # Windows
choco install kage             # Windows
brew install anomalyco/tap/kage # macOS и Linux (рекомендуем, всегда актуально)
brew install kage              # macOS и Linux (официальная формула brew, обновляется реже)
sudo pacman -S kage            # Arch Linux (Stable)
paru -S kage-bin               # Arch Linux (Latest from AUR)
mise use -g kage               # любая ОС
nix run nixpkgs#kage           # или github:anomalyco/kage для самой свежей ветки dev
```
...
KAGE также доступен как десктопное приложение. Скачайте его со [страницы релизов](https://github.com/anomalyco/kage/releases) или с [kage.ai/download](https://kage.ai/download).
...
4. `$HOME/.kage/bin` - Fallback по умолчанию
...
В KAGE есть два встроенных агента, между которыми можно переключаться клавишей `Tab`.
...
Подробнее об [agents](https://kage.ai/docs/agents).

### Документация

Больше информации о том, как настроить KAGE: [**наши docs**](https://kage.ai/docs).

### Вклад

Если вы хотите внести вклад в KAGE, прочитайте [contributing docs](./CONTRIBUTING.md) перед тем, как отправлять pull request.

### Разработка на базе KAGE

Если вы делаете проект, связанный с KAGE, и используете "kage" как часть имени (например, "kage-dashboard" или "kage-mobile"), добавьте примечание в README, чтобы уточнить, что проект не создан командой KAGE и не аффилирован с нами.

---

**Присоединяйтесь к нашему сообществу** [Discord](https://discord.gg/kage) | [X.com](https://x.com/kage)
