...
KAGE Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u KAGE-u. Pretplatite se na KAGE Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili KAGE.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa KAGE-om.
:::
...
KAGE Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

KAGE Go radi kao bilo koji drugi provajder u KAGE-u.

1. Prijavite se na **<a href={console}>KAGE Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `KAGE Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na KAGE Go.
:::
...
KAGE Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj KAGE konfiguraciji
koristi format `kage-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`kage-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo KAGE Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz KAGE koristite i bilo kojeg drugog provajdera.
