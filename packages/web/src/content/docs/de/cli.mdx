---
title: CLI
description: KAGE CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der KAGE CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
kage
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit KAGE interagieren.
...
Starten Sie die Terminalbenutzeroberfläche KAGE.
...
Der KAGE CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für KAGE verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden KAGE-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
kage attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend KAGE. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `kage`) |
...
KAGE wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `kage auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/kage/auth.json` gespeichert.

```bash
kage auth login
```

Wenn KAGE startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in KAGE sehen möchten.
...
Führen Sie KAGE im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `kage serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden KAGE-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `kage`)                  |
...
Starten Sie einen Headless-KAGE-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
kage serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf KAGE-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `kage`).
...
Listen Sie alle KAGE-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre KAGE-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer KAGE-Freigabe-URL.

```bash
kage import <file>
```

Sie können aus einer lokalen Datei oder einer KAGE-Freigabe-URL importieren.
...
Starten Sie einen Headless-KAGE-Server mit einer Weboberfläche.

```bash
kage web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf KAGE zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `kage`).
...
Deinstallieren Sie KAGE und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert KAGE auf die neueste Version oder eine bestimmte Version.
...
Der KAGE CLI akzeptiert die folgenden globalen Flags.
...
KAGE kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `kage`) |
