...
Umiejętności agenta pozwalają kage odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
kage przeszukuje te lokalizacje:

- Project config: `.kage/skills/<name>/SKILL.md`
- Global config: `~/.config/kage/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu kage przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.kage/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.kage/skills/git-release/SKILL.md` w ten sposób:
...
kage wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `kage.json`:
...
**Dla agentów wbudowanych** (w `kage.json`):
