---
title: CLI
description: أوامر وخيارات CLI في KAGE.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ KAGE افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
kage
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع KAGE برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ KAGE.
...
يتضمن CLI الخاص بـ KAGE أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ KAGE قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
kage attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ KAGE تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `kage`) |
...
يعتمد KAGE على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `kage auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

عند تشغيل KAGE يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في KAGE.
...
تشغيل kage في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `kage serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم kage قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `kage`) |
...
بدء خادم KAGE بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
kage serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف kage دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `kage`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات KAGE لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة KAGE.

```bash
kage import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة KAGE.
...
بدء خادم KAGE بدون واجهة مع واجهة ويب.

```bash
kage web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى KAGE عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `kage`).
...
إلغاء تثبيت KAGE وإزالة جميع الملفات ذات الصلة.
...
تحديث kage إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر kage الخيارات العامة التالية.
...
يمكن تهيئة KAGE باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `kage`) |
