---
title: TUI
description: Bruke kage-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KAGE gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre KAGE starter TUI for gjeldende katalog.
...
Når du bruker KAGE TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til KAGE. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Dette er atskilt fra `kage.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge KAGE venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `kage.default`.
