...
KAGE Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, KAGE'daki diğer sağlayıcılar gibi çalışır. KAGE Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve KAGE'u kullanmak için bunu kullanmanıza gerek yoktur.
...
KAGE Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

KAGE Go, KAGE'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>KAGE Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `KAGE Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye KAGE Go'ya abone olabilir.
:::
...
KAGE Go aşağıdaki limitleri içerir:
...
KAGE yapılandırmanızdaki [model id](/docs/config/#models) formatı `kage-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `kage-go/kimi-k3` kullanmalısınız.
...
KAGE Go'yu şu amaçlarla oluşturduk:
...
4. KAGE ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
