    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.kageChecks[item.config.distro]
          const kageAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-kage" && wsl.data.job.distro === item.config.distro
          return (
