---
title: TUI
description: KAGE terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KAGE, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

KAGE'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
KAGE TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
KAGE'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
KAGE'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `kage.json` dosyasından ayrıdır.
...
- `leader_timeout` - KAGE'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `kage.default`.
