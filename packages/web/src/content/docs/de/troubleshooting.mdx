...
Wenn KAGE Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\kage\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `kage --log-level DEBUG`.

---

## Speicher

kage speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/kage/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\kage`
...
KAGE Desktop startet im Hintergrund einen lokalen KAGE-Server (`kage-cli`-Sidecar).
...
- Nur macOS: `KAGE`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/kage/kage.jsonc` (oder `~/.config/kage/kage.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/kage/kage.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\kage\kage.jsonc`
...
KAGE kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/kage/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\kage\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit KAGE ihn neu aufbauen kann.

1. Beende KAGE Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/kage`
- **Linux**: loesche `~/.cache/kage` (oder fuehre aus: `rm -rf ~/.cache/kage`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\kage`

3. Starte KAGE Desktop neu.

---

### Server-Verbindungsprobleme beheben

KAGE Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `kage.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt KAGE Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit KAGE-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

KAGE Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer KAGE in den OS-Einstellungen aktiviert sind, und
...
1. Beende KAGE Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von KAGE Desktop):

- `kage.settings.dat` (desktop default server URL)
- `kage.global.dat` und `kage.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit KAGE hast:
...
3. Stelle sicher, dass du die neueste Version hast: `kage upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `kage models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\kage`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. kage installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\kage`

2. Starte kage neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt kage, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
kage erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
