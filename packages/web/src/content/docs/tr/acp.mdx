---
title: ACP Desteği
description: kage'u ACP uyumlu herhangi bir editörde kullanın.
---

kage, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
kage'u ACP aracılığıyla kullanmak için düzenleyicinizi `kage acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, kage'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'KAGE' ajanını kullanın.
...
kage'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı kage'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
kage, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- kage yapılandırmanızda yapılandırılmış MCP sunucuları
