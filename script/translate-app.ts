...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "kage/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".kage/glossary/zh-cn.md"
  if (locale === "zht") return ".kage/glossary/zh-tw.md"
  return `.kage/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("KAGE did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("KAGE returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("KAGE session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel KAGE runs for 'all' (default: 4)
      --model <provider/id>  KAGE model (default: kage/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running KAGE
...
`)
  ...
  if (failed.length) console.error(`\nKAGE failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  const exported = Bun.spawn(["kage", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OPENCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["kage", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
