...
import type { PermissionRequest } from "@kage/sdk/v2"
...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until KAGE is restarted.`]
  }

  return [
    "This will allow the following patterns until KAGE is restarted.",
