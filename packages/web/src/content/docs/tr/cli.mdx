---
title: CLI
description: kage CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

kage CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
kage
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, kage ile programlı olarak etkileşim kurmanıza olanak tanır.
...
kage terminal kullanıcı arayüzünü başlatır.
...
kage CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

kage için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir kage arka uç sunucusuna bir terminal ekleyin.

```bash
kage attach [url]
```

Bu, TUI öğesinin uzak bir kage arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `kage`) |
...
kage, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `kage auth login` kullanabilirsiniz. Bu `~/.local/share/kage/auth.json`'de saklanıyor.

```bash
kage auth login
```

kage başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları kage'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek kage'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `kage serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir kage sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `kage`) |
...
API erişimi için headless bir kage sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
kage serve
```

Bu, TUI arayüzü olmadan kage işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `kage` şeklindedir).
...
Tüm kage oturumlarını listeleyin.
...
kage oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya kage paylaşımından URL oturum verilerini içe aktarın.

```bash
kage import <file>
```

Yerel bir dosyadan veya kage paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir kage sunucusu başlatın.

```bash
kage web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla kage'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `kage` şeklindedir).
...
kage'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
kage'u en son sürüme veya belirli bir sürüme günceller.
...
kage CLI aşağıdaki global bayrakları destekler.
...
kage ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `kage`) |
