...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("kage-find")
  api.delete("kage-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("kage-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("kage-find", new Highlight(...rest))
    return true
  }
