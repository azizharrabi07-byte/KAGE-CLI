---
title: Plugin
description: Scrivi plugin per estendere KAGE.
---

I plugin ti permettono di estendere KAGE agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di KAGE.
...
- `.kage/plugins/` - plugin a livello progetto
- `~/.config/kage/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/kage/node_modules/`.
...
1. Config globale (`~/.config/kage/kage.json`)
2. Config di progetto (`kage.json`)
3. Directory plugin globale (`~/.config/kage/plugins/`)
4. Directory plugin di progetto (`.kage/plugins/`)
...
KAGE esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di kage per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere kage.
...
:::note
Se usi l'app desktop di KAGE, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a kage di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a kage:
...
L'helper `tool` crea uno strumento personalizzato che kage può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in kage insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://kage.ai/docs/sdk) per i dettagli.
