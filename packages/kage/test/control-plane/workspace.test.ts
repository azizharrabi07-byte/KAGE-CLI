...
import { EventSequenceTable } from "@kage/core/event/sql"
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@kage.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=kage-test"
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=kage-test")
