...
export async function spawnWslSidecar(
  ...
  const kage = await resolveWslOpencode(distro)
  if (!kage) throw new Error(`KAGE is not installed in ${distro}`)
  ...
  const script = [
    ...
    `exec ${shellEscape(kage)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
