---
title: Complementos
description: Escriba sus propios complementos para extender KAGE.
---

Los complementos le permiten extender KAGE al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de KAGE.
...
- `.kage/plugins/` - Complementos a nivel de proyecto
- `~/.config/kage/plugins/` - Complementos globales
...
```json title="kage.json"
{
  ...
  "plugin": ["kage-helicone-session", "kage-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/kage/node_modules/`.
...
1. Configuración global (`~/.config/kage/kage.json`)
2. Configuración del proyecto (`kage.json`)
3. Directorio global de complementos (`~/.config/kage/plugins/`)
4. Directorio de complementos del proyecto (`.kage/plugins/`)
...
KAGE ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".kage/plugins/my-plugin.ts"
...
```js title=".kage/plugins/example.js"
...
- `client`: Un cliente SDK kage para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar kage.
...
```js title=".kage/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "kage"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio KAGE, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que kage lea archivos `.env`:

```javascript title=".kage/plugins/env-protection.js"
...
```javascript title=".kage/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a kage:

```ts title=".kage/plugins/custom-tools.ts"
import { type Plugin, tool } from "@kage-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que kage puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para kage junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://kage.ai/docs/sdk) para obtener más detalles.
...
```ts title=".kage/plugins/compaction.ts"
import type { Plugin } from "@kage-ai/plugin"
...
```ts title=".kage/plugins/custom-compaction.ts"
import type { Plugin } from "@kage-ai/plugin"
