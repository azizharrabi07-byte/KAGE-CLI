...
import { Global } from "@kage-ai/core/global"
import { Glob } from "@kage-ai/core/util/glob"
...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".kage"))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "kage")
        draft.active = typeof active === "string" ? active : "kage"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "kage")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.kage, store.mode)
    })
