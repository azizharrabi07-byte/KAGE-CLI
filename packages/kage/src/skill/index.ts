...
import { SkillPlugin } from "@kage/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@kage/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@kage/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@kage/core/util/glob"
...
// Built-in skill that ships with kage. The model's intuition for what an
// kage.json should look like is often wrong, and kage hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch kage's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_OPENCODE_SKILL_NAME = "customize-kage"
const CUSTOMIZE_OPENCODE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating kage's own configuration: kage.json, kage.jsonc, files under .kage/, or files under ~/.config/kage/. Also use when creating or fixing kage agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring kage itself."
...
export class Service extends Context.Service<Service, Interface>()("@kage/Skill") {}
