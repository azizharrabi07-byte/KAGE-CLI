---
title: CLI
description: Options et commandes de la CLI KAGE.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI KAGE démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
kage
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec KAGE par programme.
...
Démarrez l’interface utilisateur du terminal KAGE.
...
La CLI KAGE dispose également des commandes suivantes.

---

### agent

Gérer les agents pour KAGE.
...
Attachez un terminal à un serveur backend KAGE déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
kage attach [url]
```

Cela permet d'utiliser le TUI avec un backend KAGE distant. Par exemple :
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `kage`) |
...
KAGE est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `kage auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Lorsque KAGE démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans KAGE.
...
Exécutez kage en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `kage serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Connectez-vous à un serveur kage en cours d'exécution (par exemple, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `OPENCODE_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `kage`) |
...
Démarrez un serveur KAGE sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
kage serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'KAGE sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `kage`).
...
Liste toutes les sessions KAGE.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions KAGE.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage KAGE.

```bash
kage import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage KAGE.
...
Démarrez un serveur KAGE sans interface graphique avec une interface Web.

```bash
kage web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à KAGE via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `kage`).
...
Désinstallez KAGE et supprimez tous les fichiers associés.
...
Met à jour kage vers la dernière version ou une version spécifique.
...
La CLI kage prend les flags globaux suivants.
...
KAGE peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `kage`) |
