...
Ajan becerileri, kage'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
kage su konumlari tarar:

- Proje konfigurasyonu: `.kage/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/kage/skills/<name>/SKILL.md`
...
Proje yerel yollarinda kage, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.kage/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.kage/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
kage, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`kage.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`kage.json` icinde):
