---
title: Formatadores
description: O kage usa formatadores específicos de linguagem.
---

O kage formata automaticamente arquivos após serem escritos ou editados usando formatadores específicos de linguagem. Isso garante que o código gerado siga os estilos de código do seu projeto.

---

## Integrados

O kage vem com vários formatadores integrados para linguagens e frameworks populares. Abaixo está uma lista dos formatadores, extensões de arquivo suportadas e comandos ou opções de configuração necessárias.
...
Portanto, se seu projeto tiver `prettier` em seu `package.json`, o kage o usará automaticamente.

---

## Como funciona

Quando o kage escreve ou edita um arquivo, ele:
...
Você pode personalizar os formatadores através da seção `formatter` em sua configuração do kage.
