...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez KAGE.
...
Voici plusieurs modèles qui fonctionnent bien avec KAGE, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration KAGE.

```json title="kage.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [KAGE Zen](/docs/zen), vous utiliserez `kage/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="kage.jsonc" {7-12,19-24}
{
  "$schema": "https://kage.ai/config.json",
  "provider": {
    "openai": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. KAGE est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

KAGE est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque KAGE démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration KAGE.
