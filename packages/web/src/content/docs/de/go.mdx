...
KAGE Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in KAGE. Du abonnierst KAGE Go und
...
KAGE Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

KAGE Go funktioniert wie jeder andere Provider in KAGE.

1. Du meldest dich bei **<a href={console}>KAGE Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `KAGE Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann KAGE Go abonnieren.
:::
...
KAGE Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner KAGE Config verwendet das Format `kage-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `kage-go/kimi-k3` in deiner Config verwenden.
...
Wir haben KAGE Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit KAGE zu nutzen.
