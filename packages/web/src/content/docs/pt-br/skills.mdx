...
As habilidades do agente permitem que o kage descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O kage pesquisa nesses locais:

- Configuração do projeto: `.kage/skills/<name>/SKILL.md`
- Configuração global: `~/.config/kage/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o kage sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.kage/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.kage/skills/git-release/SKILL.md` assim:
...
O kage lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `kage.json`:
...
**Para agentes embutidos** (em `kage.json`):
