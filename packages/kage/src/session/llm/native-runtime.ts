...
import type { LLMClientShape } from "@kage/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("kage"))
    return { type: "unsupported", reason: "provider is not openai, kage, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @kage/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains kage-owned. The native runtime only adapts
      // the @kage/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
