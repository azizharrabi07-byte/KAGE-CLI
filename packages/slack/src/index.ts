...
import { createOpencode, type ToolPart } from "@kage/sdk"
...
console.log("🚀 Starting kage server...")
const kage = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await kage.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new kage session...")
    const { client, server } = kage
    ...
    console.log("✅ Created kage session:", createResult.data.id)
  ...
  console.log("📝 Sending to kage:", message.text)
