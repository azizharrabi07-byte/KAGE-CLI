...
_Model Bağlam Protokolü_ veya MCP kullanarak kage'a harici araçlar ekleyebilirsiniz. kage hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [kage Config](https://kage.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/kage` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
kage, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde kage şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa kage, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `kage mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından kage, belirteçleri `~/.local/share/kage/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra kage'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi kage yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve kage'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
