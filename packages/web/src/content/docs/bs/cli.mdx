---
title: CLI
description: KAGE CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KAGE CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
kage
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa KAGE.
...
Pokrenite KAGE terminalski korisnički interfejs.
...
KAGE CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte KAGE agentima.
...
Priključite terminal na već pokrenut KAGE backend server pokrenut putem `serve` ili `web` naredbi.

```bash
kage attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim KAGE backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `kage`) |
...
KAGE pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `kage auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Kada se KAGE pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u KAGE.
...
Pokrenite KAGE u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `kage serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti KAGE server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `kage`) |
...
Pokrenite KAGE headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
kage serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti KAGE-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `kage`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše KAGE sesije.
...
Uvezite podatke sesije iz JSON datoteke ili KAGE dijeljenog URL-a.

```bash
kage import <file>
```

Možete uvesti iz lokalne datoteke ili KAGE dijeljenog URL-a.
...
Pokrenite KAGE headless server sa web interfejsom.

```bash
kage web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup KAGE-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `kage`).
...
Deinstalirajte KAGE i uklonite sve povezane datoteke.
...
Ažurira KAGE na najnoviju verziju ili određenu verziju.
...
KAGE CLI prihvata sljedeće globalne zastavice.
...
KAGE se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `kage`) |
