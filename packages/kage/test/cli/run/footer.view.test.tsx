...
import type { QuestionRequest } from "@kage-ai/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@kage-ai/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "kage", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "kage", modelID: "gpt-5" })
