---
title: Plugins
description: Écrivez vos propres plugins pour étendre KAGE.
---

Les plugins vous permettent d'étendre KAGE en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de KAGE.
...
- `.kage/plugins/` - Plugins au niveau du projet
- `~/.config/kage/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/kage/node_modules/`.
...
1. Configuration globale (`~/.config/kage/kage.json`)
2. Configuration du projet (`kage.json`)
3. Répertoire global des plugins (`~/.config/kage/plugins/`)
4. Répertoire des plugins du projet (`.kage/plugins/`)
...
KAGE exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK kage pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre kage.
...
:::note
Si vous utilisez l'application de bureau KAGE, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez kage de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à kage :
...
L'assistant `tool` crée un outil personnalisé que kage peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour kage aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://kage.ai/docs/sdk) pour plus de détails.
