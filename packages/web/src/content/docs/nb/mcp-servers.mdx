...
Du kan legge til eksterne verktøy i KAGE ved å bruke _Model Context Protocol_ eller MCP. KAGE støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [KAGE Config](https://kage.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="kage.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/kage`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
KAGE håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil KAGE:
...
Hvis serveren krever autentisering, vil KAGE be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `kage mcp auth <server-name>`.
...
```bash
kage mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil KAGE lagre tokens sikkert i `~/.local/share/kage/mcp-auth.json`.
...
kage mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i KAGE, sammen med innebygde verktøy. Så du kan administrere dem gjennom KAGE-konfigurasjonen som alle andre verktøy.
...
```bash
kage mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble KAGE til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver KAGE-samtale oppført som en separat økt.
