...
description: Brug KAGE i enhver ACP-kompatibel editor.
---

KAGE understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge KAGE via ACP, konfigurer din editor til at køre kommandoen `kage acp`.

Kommandoen starter KAGE som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'KAGE'-agent i AI Chat-agentvælgeren.
...
For at bruge KAGE som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge KAGE som ACP-agent til chat.
...
KAGE fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din KAGE-konfiguration
