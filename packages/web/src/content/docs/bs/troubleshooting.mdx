...
Da biste otklonili probleme s KAGE, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\kage\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `kage --log-level DEBUG`.

---

## Pohrana

KAGE pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/kage/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\kage`
...
KAGE Desktop pokreće lokalni KAGE server (`kage-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `KAGE` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/kage/kage.jsonc` (ili `~/.config/kage/kage.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/kage/kage.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\kage\kage.jsonc`
...
KAGE također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/kage/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\kage\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga KAGE mogao ponovo izgraditi.

1. Potpuno zatvorite KAGE Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/kage`
- **Linux**: obrišite `~/.cache/kage` (ili pokrenite `rm -rf ~/.cache/kage`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\kage`

3. Ponovo pokrenite KAGE Desktop.

---

### Rješavanje problema sa vezom na serveru

KAGE Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `kage.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, KAGE Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa KAGE karakteristikama.

---

### Obavještenja se ne prikazuju

KAGE Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za KAGE u postavkama vašeg OS-a, i
...
1. Zatvorite KAGE Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka KAGE Desktop aplikacije):

- `kage.settings.dat` (URL zadanog servera za desktop)
- `kage.global.dat` i `kage.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `kage upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `kage models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\kage`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. KAGE dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\kage`

2. Ponovo pokrenite KAGE da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti KAGE da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
KAGE će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
