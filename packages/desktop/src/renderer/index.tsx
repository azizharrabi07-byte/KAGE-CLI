...
import type { UpdaterState } from "@kage-ai/app/updater"
...
const deepLinkEvent = "kage:deep-link"
...
function windowLastActiveUrlKey(windowID: string) {
  return `kage.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://kage.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("kage.global.dat").getItem("language")
