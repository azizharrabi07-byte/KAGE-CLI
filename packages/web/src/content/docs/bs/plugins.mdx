---
title: Dodaci
description: Napišite vlastite dodatke za proširenje KAGE.
---

Dodaci vam omogućavaju da proširite KAGE spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja KAGE.
...
- `.kage/plugins/` - Dodaci na nivou projekta
- `~/.config/kage/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/kage/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/kage/kage.json`)
2. Konfiguracija projekta (`kage.json`)
3. Globalni direktorij dodataka (`~/.config/kage/plugins/`)
4. Direktorij dodataka projekta (`.kage/plugins/`)
...
KAGE pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje KAGE.
...
Spriječite kage da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u kage:
...
Pomoćnik `tool` kreira prilagođeni alat koji kage može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za kage zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://kage.ai/docs/sdk) za detalje.
