...
Trim `packages/kage/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/kage/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/kage`.
- `[x]` `bun typecheck` from `packages/kage`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/kage/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/kage`.
- `[x]` `bun dev generate > /tmp/kage-openapi.json` from `packages/kage`.
...
- Edit `packages/kage/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/kage-openapi.json` from `packages/kage`.
...
- Check source schemas in files like `packages/kage/src/session/schema.ts`, `packages/kage/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/kage/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/kage/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
