...
Avec KAGE, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, KAGE utilise notre propre thème `kage`.
...
KAGE est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que KAGE corresponde à l'apparence de leur terminal
...
KAGE prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/kage/themes/*.json` ou `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.kage/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.kage/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kage/themes
vim ~/.config/kage/themes/my-theme.json
```
...
{
  "$schema": "https://kage.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
