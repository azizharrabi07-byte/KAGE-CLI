---
title: CLI
description: Opzioni e comandi della CLI di KAGE.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di KAGE, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
kage
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con KAGE in modo programmatico.
...
Avvia l'interfaccia testuale di KAGE (interfaccia utente da terminale).
...
La CLI di KAGE include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per KAGE.
...
Collega un terminale a un backend server di KAGE già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
kage attach [url]
```

Questo consente di usare la TUI con un backend KAGE remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `kage`) |
...
KAGE si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `kage auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Quando KAGE si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in KAGE.
...
Esegui kage in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `kage serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server kage in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `kage`) |
...
Avvia un server KAGE headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
kage serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di kage senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `kage`).
...
Mostra statistiche di utilizzo token e costo per le sessioni KAGE.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione KAGE.

```bash
kage import <file>
```

Puoi importare da un file locale o da un URL di condivisione KAGE.
...
Avvia un server KAGE headless con interfaccia web.

```bash
kage web
```

Avvia un server HTTP e apre un browser per accedere a KAGE tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `kage`).
...
Disinstalla KAGE e rimuove tutti i file correlati.
...
Aggiorna kage all'ultima versione o a una versione specifica.
...
La CLI di kage accetta i seguenti flag globali.
...
KAGE può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `kage`)     |
