---
title: Plugins
description: Escreva seus próprios plugins para estender o kage.
---

Plugins permitem que você estenda o kage conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do kage.
...
- `.kage/plugins/` - Plugins em nível de projeto
- `~/.config/kage/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/kage/node_modules/`.
...
1. Configuração global (`~/.config/kage/kage.json`)
2. Configuração do projeto (`kage.json`)
3. Diretório de plugins global (`~/.config/kage/plugins/`)
4. Diretório de plugins do projeto (`.kage/plugins/`)
...
O kage executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do kage para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o kage.
...
:::note
Se você estiver usando o aplicativo desktop kage, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o kage de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao kage:
...
O helper `tool` cria uma ferramenta personalizada que o kage pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o kage junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://kage.ai/docs/sdk) para detalhes.
