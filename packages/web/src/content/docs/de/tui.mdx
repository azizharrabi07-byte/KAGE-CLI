---
title: TUI
description: Arbeite mit KAGE in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KAGE bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet KAGE die TUI im aktuellen Verzeichnis.
...
In der KAGE-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu KAGE hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Dies ist getrennt von `kage.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange KAGE nach der leader key wartet. Standard ist `2000`.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `kage.default`.
