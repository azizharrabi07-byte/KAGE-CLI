...
const TUI_SCHEMA_URL = "https://kage.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from kage.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const kage = await kageFiles(input)
  for (const file of kage) {
...
async function kageFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "kage"),
    ...(await Filesystem.findUp(["kage.json", "kage.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "kage"))
  }
