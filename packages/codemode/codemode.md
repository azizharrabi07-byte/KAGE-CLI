...
This is the living design and status document for `@kage-ai/codemode` and its existing V2 KAGE adapter.
...
`packages/kage/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
