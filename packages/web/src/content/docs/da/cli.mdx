---
title: CLI
description: KAGE CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KAGE CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
kage
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med KAGE programmatisk.
...
Start KAGE terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende KAGE backend-server startet via `serve` eller `web` kommandoer.

```bash
kage attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern KAGE backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `kage`) |
...
KAGE drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `kage auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Når KAGE starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i KAGE.
...
Kør kage i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `kage serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende kage-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `kage`)   |
...
Start en hovedløs KAGE-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
kage serve
```

Dette starter en HTTP-server, der giver API-adgang til kage-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `kage`).
...
Administrator KAGE sessionsholder.
...
Liste over alle KAGE sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine KAGE-sessioner.
...
Importer sessionsdata fra en JSON fil eller KAGE del URL.
...
Start en hovedløs KAGE-server med en webgrænseflade.

```bash
kage web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til KAGE via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `kage`).
...
Afinstaller KAGE og fjern alle relaterede filer.
...
Opdaterer KAGE til den nyeste version eller en specifik version.
...
kage CLI tager følgende globale flag.
...
KAGE kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `kage`) |
