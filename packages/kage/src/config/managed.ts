...
const MANAGED_PLIST_DOMAIN = "ai.kage.managed"

// Keys injected by macOS/MDM into the managed plist that are not KAGE config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/kage"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "kage")
    default:
      return "/etc/kage"
  }
}
