---
title: Supporto ACP
description: Usa KAGE in qualunque editor compatibile con ACP.
---

KAGE supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare KAGE via ACP, configura l'editor in modo che esegua il comando `kage acp`.

Il comando avvia KAGE come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'KAGE' nel selettore agenti di AI Chat.
...
Per usare KAGE come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare KAGE come agente ACP per la chat.
...
KAGE funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione KAGE
