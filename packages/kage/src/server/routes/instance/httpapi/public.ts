...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for kage serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(KAGEHttpApi)
...
export const PublicApi = KAGEHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "kage",
    version: "1.0.0",
    description: "kage api",
