...
With KAGE you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, KAGE uses our own `kage` theme.
...
KAGE comes with several built-in themes.
...
- Want KAGE to match their terminal's appearance
...
KAGE supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kage/themes/*.json` or `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kage/themes/*.json`
4. **Current working directory** - Defined in `./.kage/themes/*.json`
