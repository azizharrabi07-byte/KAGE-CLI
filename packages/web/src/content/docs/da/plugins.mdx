---
title: Plugins
description: Skriv dine egne plugins for at udvide KAGE.
---

Plugins giver dig mulighed for at udvide KAGE ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre KAGEs standardadfærd.
...
- `.kage/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/kage/node_modules/`.
...
1. Global konfiguration (`~/.config/kage/kage.json`)
2. Projektkonfiguration (`kage.json`)
3. Global plugin-mappe (`~/.config/kage/plugins/`)
4. Projekt plugin bibliotek (`.kage/plugins/`)
...
KAGE kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En kage SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide kage.
...
:::note
Hvis du bruger KAGE desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå kage i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til kage:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som kage kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for kage sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://kage.ai/docs/sdk) for detaljer.
