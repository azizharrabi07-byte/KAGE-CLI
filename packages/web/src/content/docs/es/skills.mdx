...
Las habilidades del agente permiten a KAGE descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
KAGE busca estas ubicaciones:

- Configuración del proyecto: `.kage/skills/<name>/SKILL.md`
- Configuración global: `~/.config/kage/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, KAGE sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.kage/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.kage/skills/git-release/SKILL.md` así:
...
KAGE enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `kage.json`:
...
**Para agentes integrados** (en `kage.json`):
