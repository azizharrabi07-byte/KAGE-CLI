...
Mit KAGE kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet KAGE das `kage`-Theme.
...
KAGE liefert mehrere eingebaute Themes mit.
...
- KAGE optisch ans Terminal anpassen wollen
...
KAGE unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/kage/themes/*.json` oder `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.kage/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.kage/themes/*.json`
