---
title: TUI
description: Using the KAGE terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KAGE provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running KAGE starts the TUI for the current directory.
...
When using the KAGE TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to KAGE. Allows you to select from available providers and add their API keys.
...
This is separate from `kage.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long KAGE waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `kage.default`.
