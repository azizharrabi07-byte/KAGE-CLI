...
Agentferdigheter lar KAGE oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
KAGE søker etter disse stedene:

- Prosjektkonfigurasjon: `.kage/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/kage/skills/<name>/SKILL.md`
...
For prosjektlokale stier går KAGE opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.kage/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.kage/skills/git-release/SKILL.md` slik:
...
KAGE viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `kage.json`:
...
**For innebygde agenter** (i `kage.json`):
