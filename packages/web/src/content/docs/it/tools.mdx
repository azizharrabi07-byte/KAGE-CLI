...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. KAGE include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in KAGE.
...
:::note
Questo strumento e' disponibile solo quando usi il provider KAGE o quando la variabile d'ambiente `OPENCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di KAGE:
