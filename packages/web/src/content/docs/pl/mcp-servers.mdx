...
Możesz dodać dodatkowe informacje do kage za pomocą _Model Context Protocol_, w skrócie MCP. kage obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [kage Config](https://kage.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/kage`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
kage automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, kage:
...
Jeśli serwer wymaga uwierzytelnienia, kage poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `kage mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji kage bezpieczne przechowa tokeny w `~/.local/share/kage/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w kage, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie kage, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć kage z kontem Sentry.
