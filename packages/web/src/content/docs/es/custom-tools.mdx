...
description: Cree herramientas que LLM pueda llamar en kage.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de kage como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.kage/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/kage/tools/`.
...
```ts title=".kage/tools/database.ts" {1}
...
```ts title=".kage/tools/project.ts" {8}
import { tool } from "@kage-ai/plugin"
...
```python title=".kage/tools/add.py"
import sys
...
```ts title=".kage/tools/python-add.ts" {10}
import { tool } from "@kage-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".kage/tools/add.py")
