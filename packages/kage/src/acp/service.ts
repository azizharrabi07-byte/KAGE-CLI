...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@kage/sdk/v2"
...
export const AuthMethodID = "kage-login"
...
export class Service extends Context.Service<Service, Interface>()("@kage/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `kage auth login` in the terminal",
      name: "Login with kage",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, kage provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const kageProvider = providers[ProviderV2.ID.make("kage")]
  const kageModel = kageProvider ? Provider.sort(Object.values(kageProvider.models))[0] : undefined
  if (kageProvider && kageModel) return { providerID: kageProvider.id, modelID: kageModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "KAGE service failure", service })
}
