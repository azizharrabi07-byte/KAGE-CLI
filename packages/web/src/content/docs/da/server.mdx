---
title: Server
description: Interager med kage-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `kage serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en kage-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `kage.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `kage`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `kage serve` og `kage web`.
...
Når du kører `kage` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug kage-serveren til at interagere med kage programmatisk.
:::

Denne arkitektur lader kage understøtte flere klienter og lader dig interagere med kage programmatisk.

Du kan køre `kage serve` for at starte en selvstændig server. Hvis du har
kage TUI kørende, vil `kage serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af KAGE [IDE](/docs/ide) plugins.
...
KAGE-serveren viser følgende API'er.
