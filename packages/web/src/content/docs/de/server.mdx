---
title: Server
description: Nutze den kage-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `kage serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein kage-Client nutzen kann.
...
Der Benutzername ist standardmaessig `kage`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `kage serve` und `kage web`.
...
Wenn du `kage` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den kage-Server, um kage programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `kage serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `kage serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die KAGE-[IDE](/docs/ide)-Plugins.
...
Der kage-Server stellt folgende APIs bereit.
