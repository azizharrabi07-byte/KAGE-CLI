---
title: Server
description: Interact with kage server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `kage serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an kage client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `kage.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `kage`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `kage serve` and `kage web`.
...
When you run `kage` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the kage server to interact with kage programmatically.
:::

This architecture lets kage support multiple clients and allows you to interact with kage programmatically.

You can run `kage serve` to start a standalone server. If you have the
kage TUI running, `kage serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the KAGE [IDE](/docs/ide) plugins.
...
The kage server exposes the following APIs.
