...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. KAGE posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w KAGE.
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy KAGE lub gdy zmienna środowiskowa `OPENCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu KAGE:
