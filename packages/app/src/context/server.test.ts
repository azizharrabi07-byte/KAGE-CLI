  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://kage.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://kage.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://kage.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://kage.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://kage.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://kage.example.com"),
      ),
    ).toEqual({
