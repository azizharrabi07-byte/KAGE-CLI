---
title: ACP Support
description: Use KAGE in any ACP-compatible editor.
---

KAGE supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use KAGE via ACP, configure your editor to run the `kage acp` command.

The command starts KAGE as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'KAGE' agent in the AI Chat agent selector.
...
To use KAGE as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use KAGE as the ACP agent for chat.
...
KAGE works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your KAGE config
