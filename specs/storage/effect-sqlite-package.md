...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an kage storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/kage` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No kage paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- Opencode-specific path/channel selection stays in `packages/kage`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later kage adoption PR.

The current `packages/kage/src/storage/db.ts` has two non-obvious semantics that the kage wrapper must preserve when it consumes this adapter:
...
The kage wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving kage to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@kage-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@kage-ai/effect-drizzle-sqlite` as a dependency of `packages/kage`.
5. Port `packages/kage/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus kage-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of kage's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then kage can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay kage-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/kage`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not kage domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/kage` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing kage's current database runtime.
