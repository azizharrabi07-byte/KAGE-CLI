...
# Pakkehåndteringer
npm i -g kage-ai@latest        # eller bun/pnpm/yarn
scoop install kage             # Windows
choco install kage             # Windows
brew install anomalyco/tap/kage # macOS og Linux (anbefalet, altid up to date)
brew install kage              # macOS og Linux (officiel brew formula, opdateres sjældnere)
sudo pacman -S kage            # Arch Linux (Stable)
paru -S kage-bin               # Arch Linux (Latest from AUR)
mise use -g kage               # alle OS
nix run nixpkgs#kage           # eller github:anomalyco/kage for nyeste dev-branch
```
...
KAGE findes også som desktop-app. Download direkte fra [releases-siden](https://github.com/anomalyco/kage/releases) eller [kage.ai/download](https://kage.ai/download).
...
KAGE har to indbyggede agents, som du kan skifte mellem med `Tab`-tasten.
...
Læs mere om [agents](https://kage.ai/docs/agents).

### Dokumentation

For mere info om konfiguration af KAGE, [**se vores docs**](https://kage.ai/docs).

### Bidrag

Hvis du vil bidrage til KAGE, så læs vores [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygget på KAGE

Hvis du arbejder på et projekt der er relateret til KAGE og bruger "kage" som en del af navnet; f.eks. "kage-dashboard" eller "kage-mobile", så tilføj en note i din README, der tydeliggør at projektet ikke er bygget af KAGE-teamet og ikke er tilknyttet os på nogen måde.

---

**Bliv en del af vores community** [Discord](https://discord.gg/kage) | [X.com](https://x.com/kage)
