---
title: Regeln
description: Lege eigene Anweisungen fuer kage fest.
---

Du kannst kage ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in kage den Befehl `/init` aus.
...
So kann kage sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht kage sie zu erweitern.
...
kage kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/kage/AGENTS.md` speichern.
Diese gelten in allen kage-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt KAGE dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/kage/AGENTS.md` existiert)
...
Beim Start sucht kage in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/kage/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `kage.json` oder in `~/.config/kage/kage.json` angeben.
...
kage parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit kage.json

Empfohlen ist das `instructions`-Feld in der `kage.json`:
...
Du kannst kage explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `kage.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
