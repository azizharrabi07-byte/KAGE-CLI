---
title: Eklentiler
description: KAGE'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek KAGE'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya KAGE'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.kage/plugins/` - Proje düzeyinde eklentiler
- `~/.config/kage/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/kage/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/kage/kage.json`)
2. Project config (`kage.json`)
3. Global eklenti dizini (`~/.config/kage/plugins/`)
4. Proje eklenti dizini (`.kage/plugins/`)
...
KAGE bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir KAGE SDK istemcisi.
...
KAGE'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
KAGE masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
KAGE'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca KAGE'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, KAGE'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra KAGE'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://kage.ai/docs/sdk) bakın.
