...
Con KAGE puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, KAGE usa il tema `kage`.
...
KAGE include diversi temi integrati.
...
- Vuole che KAGE corrisponda all'aspetto del terminale
...
KAGE supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/kage/themes/*.json` o `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.kage/themes/*.json`
4. **Directory di lavoro corrente** - in `./.kage/themes/*.json`
