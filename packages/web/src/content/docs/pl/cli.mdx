---
title: CLI
description: Opcje i polecenia wiersza poleceń KAGE.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI KAGE domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
kage
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać KAGE programowo.
...
Uruchamia interfejs terminalowy (TUI) KAGE.
...
Interfejs CLI KAGE zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami KAGE.
...
Dołącz terminal do już działającego serwera backendu KAGE uruchomionego za pomocą `serve` lub `web`.

```bash
kage attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem KAGE. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `kage`) |
...
KAGE integruje się z wieloma dostawcami. Możesz użyć `kage auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Podczas uruchamiania KAGE ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w KAGE.
...
Uruchom KAGE w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `kage serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera KAGE (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `kage`) |
...
Uruchom serwer KAGE (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
kage serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API KAGE bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `kage`).
...
Pokaż statystyki wykorzystania tokenów i sesji KAGE.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania KAGE.
...
Uruchom serwer KAGE z interfejsem internetowym.

```bash
kage web
```

Uruchamia to serwer HTTP i udostępnia KAGE przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `kage`).
...
Odinstaluj KAGE i usuń wszystkie powiązane pliki.
...
Aktualizuj KAGE do najnowszej lub określonej wersji.
...
Interfejs CLI KAGE przyjmuje następujące flagi globalne dla każdego polecenia.
...
KAGE można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `kage`)    |
