---
title: Serveur
description: Interagissez avec le serveur kage via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `kage serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client kage peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `kage.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `kage`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `kage serve` et à `kage web`.
...
Lorsque vous exécutez `kage`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur kage pour interagir avec kage par programmation.
:::

Cette architecture permet à kage de prendre en charge plusieurs clients et vous permet d'interagir avec kage par programme.

Vous pouvez exécuter `kage serve` pour démarrer un serveur autonome. Si vous avez le
kage TUI en cours d'exécution, `kage serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins KAGE [IDE](/docs/ide).
...
Le serveur kage expose les API suivantes.
