---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w kage.
---

import config from "../../../../config.mjs"
export const console = config.console

kage używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji kage.
...
w `~/.local/share/kage/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim kage
config.
...
KAGE Zen to lista modeli dostarczonych przez zespół kage, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z kage. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od KAGE Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `KAGE Zen` i przejdź do [kage.ai/auth](https://kage.ai/zen).
...
Działa jak każdy inny dostawca w kage i jest całkowicie opcjonalny w użyciu.

---

## KAGE Go

KAGE Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół kage, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z kage.

1. Uruchom polecenie `/connect` w TUI, wybierz `KAGE Go` i przejdź do [kage.ai/auth](https://kage.ai/zen).
...
Aby używać Amazon Bedrock z kage:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania kage:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `kage.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w kage nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować kage do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod kage działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji kage.
...
kage używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować kage
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`kage.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem kage:
...
Aby używać Google Vertex AI z kage:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania kage.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą kage, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session), aby automatycznie rejestrować każdą konwersację kage jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja kage będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować kage tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować kage tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować kage tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem kage. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/kage).
:::
...
Aby korzystać z Ollama Cloud z kage:
...
3. Skopiuj klucz API do użycia w kage.
...
6. **Ważne**: Przed użyciem modeli chmurowych w kage musisz pobrać informacje o modelu lokalnie:
...
KAGE Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół kage. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>KAGE Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **KAGE Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API kage.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji kage.
...
5. Możesz je także dostosować za pomocą konfiguracji kage. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z kage:
...
Możesz także dostosować modele za pomocą konfiguracji kage. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z kage. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `kage.json` w katalogu projektu:
...
Pola `limit` pozwalają kage zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `kage auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację kage i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji kage.
