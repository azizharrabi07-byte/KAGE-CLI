...
Para depurar problemas con KAGE, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\kage\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `kage --log-level DEBUG`.

---

## Almacenamiento

kage almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/kage/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\kage`
...
KAGE Desktop ejecuta un servidor KAGE local (el sidecar `kage-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `KAGE` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/kage/kage.jsonc` (o `~/.config/kage/kage.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/kage/kage.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\kage\kage.jsonc`
...
KAGE también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/kage/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\kage\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que KAGE pueda reconstruirlo.

1. Salga de KAGE Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/kage`
- **Linux**: eliminar `~/.cache/kage` (o ejecutar `rm -rf ~/.cache/kage`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\kage`

3. Reinicie el escritorio KAGE.

---

### Solucionar problemas de conexión del servidor

KAGE Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `kage.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio KAGE requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de KAGE.

---

### Notificaciones que no se muestran

KAGE Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para KAGE en la configuración de su sistema operativo, y
...
1. Salga del escritorio KAGE.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio KAGE):

- `kage.settings.dat` (URL del servidor predeterminado de escritorio)
- `kage.global.dat` y `kage.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/kage/issues**](https://github.com/anomalyco/kage/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**kage.ai/discord**](https://kage.ai/discord)
...
3. Asegúrese de tener la última versión con `kage upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `kage models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\kage`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. kage instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\kage`

2. Reinicie kage para reinstalar los paquetes del proveedor más recientes.

Esto obligará a kage a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
kage detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
