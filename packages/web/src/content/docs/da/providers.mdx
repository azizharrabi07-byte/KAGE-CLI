---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i KAGE.
---

import config from "../../../../config.mjs"
export const console = config.console

KAGE bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i KAGE-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/kage/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i KAGE-konfigurationen.
...
KAGE Zen er en liste over modeller leveret af KAGE-teamet, som er blevet testet og verificeret til at fungere godt med KAGE. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med KAGE Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg kage og gå til [kage.ai/auth](https://kage.ai/auth).
...
Det fungerer som alle andre udbydere i KAGE og er helt valgfrit at bruge.

---

## KAGE Go

KAGE Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af KAGE-teamet, som er testet og verificeret til at fungere godt med KAGE.

1. Kør kommandoen `/connect` i TUI, vælg `KAGE Go`, og gå til [kage.ai/auth](https://kage.ai/zen).
...
Sådan bruger du Amazon Bedrock med KAGE:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører kage:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `kage.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i KAGE understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere kage til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at kage skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem kage-konfigurationen.
...
gemme tokenet i kage auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
KAGE bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse KAGE
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`kage.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `kage.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med KAGE:
...
Sådan bruger du Google Vertex AI med KAGE:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem kage, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session) plugin for automatisk at logge hver KAGE-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver KAGE-samtale opført som en separat session.
...
Du kan konfigurere kage til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere kage til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere kage til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for KAGE. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/kage) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med KAGE:
...
3. Kopier API-nøglen til brug i KAGE.
...
6. **Vigtigt**: Før du bruger skymodeller i KAGE, skal du hente modelinformationen lokalt:
...
KAGE Zen er en liste over testede og verificerede modeller leveret af KAGE-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>KAGE Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **KAGE Zen**.

   ```txt
   /connect
   ```

3. Indtast KAGE API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem kage-konfigurationen.
...
5. Du kan også tilpasse dem gennem kage-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med KAGE:
...
Du kan også tilpasse modeller gennem kage-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med kage. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `kage.json`-filen i projektkataloget:
...
`limit`-felterne lader KAGE forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `kage auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek kage-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i kage-konfigurationen.
