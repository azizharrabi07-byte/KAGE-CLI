...
Создайте `.kage/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию kage или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем kage [config](/docs/config):
...
- Глобальный: `~/.config/kage/commands/`
- Для каждого проекта: `.kage/commands/`
...
kage включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
