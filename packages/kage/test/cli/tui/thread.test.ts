...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ kage }) =>
    Effect.gen(function* () {
      const result = yield* kage.spawn(["--replay-limit", "10"])

      kage.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ kage }) =>
    Effect.gen(function* () {
      const result = yield* kage.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ kage }) =>
    Effect.gen(function* () {
      const result = yield* kage.spawn(["--mini", "--port", "4096"])
