---
title: Partager
description: Partagez vos conversations KAGE.
---

La fonctionnalité de partage de KAGE vous permet de créer des liens publics vers vos conversations KAGE, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, KAGE :
...
KAGE prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, KAGE utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
...
Pour appliquer cela au sein de votre équipe pour un projet donné, ajoutez-le au `kage.json` dans votre projet et enregistrez-vous dans Git.
...
[En savoir plus](/docs/enterprise) sur l'utilisation de kage dans votre organisation.
