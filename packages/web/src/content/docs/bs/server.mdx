---
title: Server
description: Komunicirajte s kage serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `kage serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji kage klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `kage.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `kage`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `kage serve` i za `kage web`.
...
Kada pokrenete `kage`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite kage server za programsku interakciju sa kage-om.
:::

Ova arhitektura omogucava kage podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `kage serve` da startate standalone server. Ako je kage TUI vec pokrenut, `kage serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste KAGE [IDE](/docs/ide) pluginovi.
...
kage server izlaže sljedece API-je.
