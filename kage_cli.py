...
Supports Telegram Bot management (kage telegram start|stop|status), slash commands,
up-arrow history, dynamic config editing, and standardized exit codes.
...
Subcommands:
  ...
  kage telegram start|stop|status (Manage Telegram Bot process)
...
TELEGRAM_PID_FILE = Path.home() / ".kage" / "telegram.pid"
...
│  KAGE OS — Personal AI Operating System for Termux           │
...
def show_slash_agents():
    ...
    if res.get("status") == "done":
        ...
        print("─" * 70)
        for a in agents:
            st_col = C_GREEN if a.get('status') == 'awake' else C_DIM
            print(f"{a['name']:<15} {st_col}{a.get('status', 'sleeping'):<10}{C_RESET} {a.get('description', '')}")
    else:
        print(f"{C_RED}Error loading agents: {res.get('output')}{C_RESET}")


def show_slash_traces():
    ...
    if res.get("status") == "done":
        ...
        print(f"{C_RED}Error loading traces: {res.get('output')}{C_RESET}")


def show_slash_schedules():
    ...
    if res.get("status") == "done":
        ...
        print(f"{C_RED}Error loading schedules: {res.get('output')}{C_RESET}")


def execute_chat_message(message: str):
    res = run_kage("chat", {"message": message})
    if res.get("status") == "done":
        if "response" in res:
...
# --- TELEGRAM BOT SUBCOMMAND HANDLER ---

def cmd_telegram(args):
    """Manage Telegram Bot daemon process (start|stop|status)."""
    action = getattr(args, "action", "status") or "status"
    tg_script = KAGE_DIR / "agents" / "telegram" / "agent.py"
    ...
    if action == "start":
        if TELEGRAM_PID_FILE.exists():
            try:
                ...
                os.kill(pid, 0)
                print(f"{C_GREEN}[TELEGRAM BOT] Already active (PID {pid}){C_RESET}")
                sys.exit(0)
            except Exception:
                try:
        ...
        print(f"{C_CYAN}[TELEGRAM BOT] Starting background long-polling daemon (@Mini_kage_bot)...{C_RESET}")
        proc = subprocess.Popen(
            [PYTHON, str(tg_script)],
            cwd=str(KAGE_DIR),
        ...
        print(f"{C_GREEN}✅ Telegram bot started successfully in background (PID {proc.pid}){C_RESET}")
        sys.exit(0)

    elif action == "stop":
        if not TELEGRAM_PID_FILE.exists():
            print(f"{C_YELLOW}[TELEGRAM BOT] Process is not currently active.{C_RESET}")
            sys.exit(0)
        ...
        try:
            ...
            TELEGRAM_PID_FILE.unlink()
            print(f"{C_GREEN}✅ Telegram bot stopped cleanly (PID {pid}){C_RESET}")
            sys.exit(0)
        except ProcessLookupError:
            if TELEGRAM_PID_FILE.exists():
                TELEGRAM_PID_FILE.unlink()
            print(f"{C_YELLOW}[TELEGRAM BOT] Process was not running (cleaned stale PID file).{C_RESET}")
            sys.exit(0)
        except Exception as e:
            print(f"{C_RED}Failed stopping Telegram bot: {e}{C_RESET}")
            sys.exit(1)
        ...
        print(f"\n{C_BOLD}┌─── TELEGRAM BOT STATUS ───┐{C_RESET}")
        print(f"│ Bot Username:  @Mini_kage_bot")
        print(f"│ Process State: {C_GREEN if is_running else C_YELLOW}{'RUNNING (PID ' + str(pid) + ')' if is_running else 'STOPPED / STANDBY'}{C_RESET}")
        print(f"│ Script Path:   {tg_script}")
        print(f"{C_BOLD}└───────────────────────────┘{C_RESET}")
        sys.exit(0 if is_running else 1)


# --- STANDALONE COMMANDS (CLI Exit Code Handlers) ---

def cmd_chat(args):
    msg = args.message if hasattr(args, "message") else str(args)
    execute_chat_message(msg)
    sys.exit(0)
...
# --- REPL SHELL ENTRYPOINT ---

def start_interactive_repl():
    """Start continuous interactive Terminal REPL shell for Termux."""
    print(ASCII_BANNER)
    print("")
...
│  {C_CYAN}/agent{C_RESET} or {C_CYAN}/agents{C_RESET}   - List registered personal domain agents
│  {C_CYAN}/traces{C_RESET}             - List recent trace execution logs
│  {C_CYAN}/schedules{C_RESET}          - List active cron schedules
│  {C_CYAN}/logs{C_RESET}               - View recent supervisor daemon execution logs
│  {C_CYAN}/clear{C_RESET}              - Clear terminal screen
│  {C_CYAN}/exit{C_RESET}               - Exit interactive session
│  {C_GREEN}<prompt>{C_RESET}            - Natural language prompt to Kage LLM Brain
{C_BOLD}└────────────────────────────────────┘{C_RESET}""")
            ...
            elif line in ("/agent", "/agents") or line.startswith("/agent "):
                show_slash_agents()
                ...
                lf = next((p for p in candidate_logs if p.exists() and p.stat().st_size > 0), None)
                if lf:
                    subprocess.run(["tail", "-n", "30", str(lf)])
                else:
                    print("⚠️ No logs found. Start supervisor first.")
...
def main():
    ...
    p_telegram = subparsers.add_parser("telegram", help="Telegram Bot daemon management")
    p_telegram.add_argument("action", choices=["start", "stop", "status"], nargs="?", default="status", help="Action")
