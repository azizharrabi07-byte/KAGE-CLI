// Subprocess test harness for the kage CLI. Spawns the real binary against
...
//
// Configuration flows through kage's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for kage.json
...
//
// Today only `kage.run` is fully wired. The shape supports adding more
// builders (`kage.serve(opts)`, `kage.acp(opts)`, `kage.auth(...)`)
...
const kageRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(kageRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `kage run`. New flags should
...
// `kage serve` is a long-lived process — it never exits on its own.
...
// `kage acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `kage serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `kage acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("kage.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("kage.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("kage.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "kage server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `kage serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("kage.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const kage: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, kage })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `kage.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "kage") {
...
// every call site. Use this for any test that needs the kage CLI fixture.
//
...
// resources (e.g. `kage.serve`) without an extra `Effect.scoped` wrapper —
