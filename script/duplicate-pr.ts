...
import { createOpencode } from "@kage-ai/sdk"
...
`)
  ...
  const kage = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await kage.client.session.create()
    const result = await kage.client.session
      .prompt({
