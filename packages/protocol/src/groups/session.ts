...
import { PromptInput } from "@kage/schema/prompt-input"
import { Session } from "@kage/schema/session"
import { Project } from "@kage/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@kage/schema/schema"
...
import { SessionEvent } from "@kage/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this KAGE process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this KAGE process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
