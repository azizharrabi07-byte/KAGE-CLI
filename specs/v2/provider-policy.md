...
Both `action` and `resource` use kage's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.kage` files is intentionally deferred until `.kage` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether kage permits an operation through its managed execution paths.
