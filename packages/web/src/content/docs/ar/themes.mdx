...
مع KAGE يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم KAGE سمتنا `kage`.
...
- يريدون أن يطابق KAGE مظهر terminal لديهم
...
يدعم KAGE نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/kage/themes/*.json` أو `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.kage/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.kage/themes/*.json`
