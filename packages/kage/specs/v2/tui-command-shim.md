...
Delete `packages/kage/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/kage/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/kage/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/kage`
- TUI plugin loader tests in `packages/kage` if runtime plugin API wiring changed
