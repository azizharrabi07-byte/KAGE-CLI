// Subprocess integration tests for `kage run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `kage.run(message, opts?)` to spawn `bun src/index.ts run ...` with
...
describe("kage run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kage.run("say hi")
        kage.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kage.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* kage.run("think", { extraArgs: ["--thinking"] })
        kage.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* kage.run("think again")
        kage.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kage.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kage.run("say hi", { format: "json" })
        kage.expectExit(result, 0)

        const events = kage.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ kage }) =>
      Effect.gen(function* () {
        const result = yield* kage.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kage.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* kage.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, kage }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* kage.run("request permission", { permission: { bash: "ask" } })
        kage.expectExit(denied, 0)
        ...
        const allowed = yield* kage.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        kage.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* kage.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        kage.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, kage }) =>
      Effect.gen(function* () {
        ...
        const server = yield* kage.serve()

        const result = yield* kage.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, kage }) =>
      Effect.gen(function* () {
        const result = yield* kage.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, kage }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* kage.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
