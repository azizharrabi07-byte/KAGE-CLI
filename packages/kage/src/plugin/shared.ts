...
export const DEPRECATED_PLUGIN_PACKAGES = ["kage-openai-codex-auth", "kage-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, kageVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(kageVersion) || semver.major(kageVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.kage
  if (typeof range !== "string") return
  if (!semver.satisfies(kageVersion, range)) {
    throw new Error(`Plugin requires kage ${range} but running ${kageVersion}`)
  }
}
