...
U KAGE mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, KAGE koristi nasu `kage` temu.
...
KAGE dolazi sa vise ugradenih tema.
...
- Zele da KAGE odgovara izgledu njihovog terminala
...
KAGE podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/kage/themes/*.json` ili `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.kage/themes/*.json`
4. **Trenutni radni direktorij** - `./.kage/themes/*.json`
