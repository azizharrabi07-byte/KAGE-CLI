---
title: Servidor
description: Interactuar con el servidor kage a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `kage serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente kage puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `kage.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `kage`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `kage serve` como a `kage web`.
...
Cuando ejecuta `kage`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor kage para interactuar con kage mediante programación.
:::

Esta arquitectura permite que kage admita múltiples clientes y le permite interactuar con kage mediante programación.

Puede ejecutar `kage serve` para iniciar un servidor independiente. Si tienes el
kage TUI ejecutándose, `kage serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos KAGE [IDE](/docs/ide).
...
El servidor kage expone las siguientes API.
