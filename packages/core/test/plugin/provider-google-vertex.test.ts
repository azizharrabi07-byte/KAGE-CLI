...
import { GoogleVertexPlugin } from "@kage/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.kage, (provider) => {
          provider.api = {
            ...
            url: "https://kage.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.kage))
