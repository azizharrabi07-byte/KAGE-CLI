---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um KAGE zu erweitern.
---

Mit Plugins können Sie KAGE erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von KAGE zu ändern.
...
- `.kage/plugins/` – Plugins auf Projektebene
- `~/.config/kage/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/kage/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/kage/kage.json`)
2. Projektkonfiguration (`kage.json`)
3. Globales Plugin-Verzeichnis (`~/.config/kage/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.kage/plugins/`)
...
KAGE führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein KAGE-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie KAGE erweitern können.
...
:::note
Wenn Sie die KAGE-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass KAGE `.env`-Dateien liest:
...
Plugins können KAGE auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das KAGE aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für KAGE zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://kage.ai/docs/sdk).
