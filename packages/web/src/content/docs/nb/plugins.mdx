...
description: Skriv dine egne programtillegg for å utvide KAGE.
---

Plugins lar deg utvide KAGE ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til KAGE.
...
- `.kage/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/kage/node_modules/`.
...
1. Global konfigurasjon (`~/.config/kage/kage.json`)
2. Prosjektkonfigurasjon (`kage.json`)
3. Global plugin-katalog (`~/.config/kage/plugins/`)
4. Prosjektpluginkatalog (`.kage/plugins/`)
...
KAGE kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En KAGE SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide KAGE.
...
:::note
Hvis du bruker KAGE-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre KAGE fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til KAGE:
...
`tool`-hjelperen lager et tilpasset verktøy som KAGE kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for KAGE sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://kage.ai/docs/sdk) for detaljer.
