...
kage ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak kage kendi `kage` temasını kullanır.
...
kage birden fazla yerleşik temayla gelir.
...
- kage'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
kage, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/kage/themes/*.json` veya `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.kage/themes/*.json`
4. **Geçerli çalışma dizini** - `./.kage/themes/*.json`
