import type { PermissionV1 } from "@kage/core/v1/permission"
import { FSUtil } from "@kage/core/fs-util"
// CLI entry point for `kage run` and `kage --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`kage --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`kage --mini --attach`): connects to a running
//      kage server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@kage/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run kage with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running kage server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'kage')",
      })
      .option("dir", {
        type: "string",
