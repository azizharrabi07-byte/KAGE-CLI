...
Le skill degli agenti permettono a KAGE di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
KAGE cerca in queste posizioni:

- Config di progetto: `.kage/skills/<name>/SKILL.md`
- Config globale: `~/.config/kage/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, KAGE risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.kage/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.kage/skills/git-release/SKILL.md` cosi':
...
KAGE elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `kage.json`:
...
**Per agenti integrati** (in `kage.json`):
