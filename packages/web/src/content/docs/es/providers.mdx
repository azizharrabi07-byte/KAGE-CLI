---
title: Proveedores
description: Usando cualquier proveedor LLM en KAGE.
---

import config from "../../../../config.mjs"
export const console = config.console

KAGE usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración KAGE.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu KAGE
configuración.
...
KAGE Zen es una lista de modelos proporcionados por el equipo KAGE que han sido
probado y verificado para funcionar bien con KAGE. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con KAGE Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione kage y diríjase a [kage.ai/auth](https://kage.ai/auth).
...
Funciona como cualquier otro proveedor en KAGE y su uso es completamente opcional.

---

## KAGE Go

KAGE Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de KAGE que han sido
probado y verificado para funcionar bien con KAGE.

1. Ejecute el comando `/connect` en TUI, seleccione `KAGE Go` y diríjase a [kage.ai/auth](https://kage.ai/zen).
...
Para usar Amazon Bedrock con KAGE:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta kage:
   ...
   Para una configuración persistente o específica del proyecto, utilice `kage.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en KAGE no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar kage para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que kage funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración kage.
...
para almacenar el token en el almacenamiento de autenticación kage.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
KAGE utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear KAGE
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `kage.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth).

##### Configuración

Personalizar a través de `kage.json`:
...
Para utilizar su suscripción GitHub Copilot con kage:
...
Para utilizar Google Vertex AI con KAGE:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta kage.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de kage, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session) para registrar automáticamente cada conversación KAGE como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación KAGE enumerada como una sesión separada.
...
Puede configurar kage para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar kage para usar modelos locales a través de LM Studio.
...
Puedes configurar kage para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para KAGE. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/kage) para obtener más detalles.
:::
...
Para usar Ollama Cloud con KAGE:
...
3. Copie la clave API para usarla en KAGE.
...
6. **Importante**: Antes de usar modelos en la nube en KAGE, debe obtener la información del modelo localmente:
...
KAGE Zen es una lista de modelos probados y verificados proporcionada por el equipo KAGE. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>KAGE Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **KAGE Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave KAGE API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración kage.
...
5. También puedes personalizarlos a través de tu configuración kage. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con KAGE:
...
También puede personalizar modelos a través de su configuración kage. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con kage. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `kage.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a KAGE comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `kage auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración kage y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración kage.
