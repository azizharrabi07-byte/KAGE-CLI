...
import { Config } from "@kage/core/config"
import { ConfigProvider } from "@kage/core/config/provider"
import { AppNodeBuilder } from "@kage/core/effect/app-node-builder"
import { LayerNode } from "@kage/core/effect/layer-node"
import { ConfigMigrateV1 } from "@kage/core/v1/config/migrate"
...
import { Policy } from "@kage/core/policy"
import { Project } from "@kage/core/project"
import { AbsolutePath } from "@kage/core/schema"
...
describe("Config", () => {
  ...
  it.live("loads kage JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "kage.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "kage.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "kage.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "kage.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "kage.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "kage.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .kage configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".kage"), { recursive: true })
            await fs.mkdir(path.join(directory, ".kage"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "kage.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "kage.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "kage.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "kage.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "kage.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".kage", "kage.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".kage", "kage.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".kage")),
              AbsolutePath.make(path.join(directory, ".kage")),
            ])
