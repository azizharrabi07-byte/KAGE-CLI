...
Com o kage, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o kage usa nosso próprio tema `kage`.
...
O kage vem com vários temas embutidos.
...
- Querem que o kage corresponda à aparência do seu terminal
...
O kage suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/kage/themes/*.json` ou `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.kage/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.kage/themes/*.json`
