...
Možete dodati vanjske alate u KAGE koristeći _Model Context Protocol_, ili MCP. KAGE podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [KAGE Config](https://kage.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/kage`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
KAGE automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, KAGE će:
...
Ako server zahtijeva autentifikaciju, KAGE će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `kage mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, KAGE sigurno čuva tokene u `~/.local/share/kage/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u KAGE, zajedno s ugrađenim alatima. Možete njima upravljati kroz KAGE konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete KAGE sa vašim Sentry nalogom.
