...
Sie können externe Tools zu KAGE hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. KAGE unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [KAGE Config](https://kage.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/kage`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
KAGE übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird KAGE Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von KAGE beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `kage mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert KAGE die Token sicher in `~/.local/share/kage/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in KAGE verfügbar. Sie können sie also wie jedes andere Tool über die KAGE-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und KAGE mit Ihrem Sentry-Konto zu verbinden.
