...
Dzięki kage możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie kage używa naszego własnego motywu `kage`.
...
kage ma kilka wbudowanych motywów.
...
- Chcą, aby KAGE pasował do wyglądu ich terminala
...
kage obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/kage/themes/*.json` lub `$XDG_CONFIG_HOME/kage/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.kage/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.kage/themes/*.json`
