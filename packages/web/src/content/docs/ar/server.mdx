---
title: الخادم
description: تفاعل مع خادم kage عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `kage serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل kage استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `kage.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `kage`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `kage serve` و `kage web`.
...
عند تشغيل `kage` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم kage للتفاعل مع kage برمجيا.
:::

تتيح هذه البنية لـ kage دعم عدة عملاء وتمكّنك من التفاعل مع kage برمجيا.

يمكنك تشغيل `kage serve` لبدء خادم مستقل. إذا كانت واجهة kage في terminal (TUI)
قيد التشغيل، فسيبدأ `kage serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات KAGE لـ [IDE](/docs/ide).
...
يعرض خادم kage واجهات API التالية.
