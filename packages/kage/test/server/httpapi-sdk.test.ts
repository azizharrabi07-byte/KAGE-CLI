...
import { createOpencodeClient } from "@kage/sdk/v2"
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".kage", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-kage-directory")).toBe(false)
        expect(request!.headers.has("x-kage-workspace")).toBe(false)
      }),
    ),
  )
