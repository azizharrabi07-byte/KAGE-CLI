...
To debug issues with KAGE, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\kage\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `kage --log-level DEBUG`.

---

## Storage

kage stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/kage/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\kage`
...
To uninstall the KAGE CLI and remove its related files, run:
...
To remove KAGE Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

KAGE Desktop runs a local KAGE server (the `kage-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `KAGE` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/kage/kage.jsonc` (or `~/.config/kage/kage.json`)
- **macOS/Linux** (older installs): `~/.local/share/kage/kage.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\kage\kage.jsonc`
...
KAGE can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/kage/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\kage\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so KAGE can rebuild it.

1. Quit KAGE Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/kage`
- **Linux**: delete `~/.cache/kage` (or run `rm -rf ~/.cache/kage`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\kage`

3. Restart KAGE Desktop.

---

### Fix server connection issues

KAGE Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `kage.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, KAGE Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with KAGE's features.

---

### Notifications not showing

KAGE Desktop only shows system notifications when:

- notifications are enabled for KAGE in your OS settings, and
...
1. Quit KAGE Desktop.
2. Find and delete these files (they live in the KAGE Desktop app data directory):

- `kage.settings.dat` (desktop default server URL)
- `kage.global.dat` and `kage.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with KAGE:
...
3. Ensure you have the latest version with `kage upgrade`
...
To figure out what models you have access to, run `kage models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\kage`
...
If you encounter API call errors, this may be due to outdated provider packages. kage dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\kage`

2. Restart kage to reinstall the latest provider packages

This will force kage to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
kage will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
