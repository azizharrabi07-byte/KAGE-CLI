...
import { LLMEvent, LLMResponse } from "@kage/llm"
import { RequestExecutor } from "@kage/llm/route"
...
const zenURL = (connection: string) => `https://console.kage.ai/proxy/connections/${connection}/v1`
...
const RECORDED_SCENARIOS = [
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["kage", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["kage", "zen", "native", "tool-loop"],
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the kage LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "kage.json"),
      JSON.stringify({ $schema: "https://kage.ai/config.json", ...scenario.config(model) }),
    ),
  )
