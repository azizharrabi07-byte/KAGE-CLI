...
npm i -g kage-ai@latest        # o bun/pnpm/yarn
scoop install kage             # Windows
choco install kage             # Windows
brew install anomalyco/tap/kage # macOS y Linux (recomendado, siempre al día)
brew install kage              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S kage            # Arch Linux (Stable)
paru -S kage-bin               # Arch Linux (Latest from AUR)
mise use -g kage               # cualquier sistema
nix run nixpkgs#kage           # o github:anomalyco/kage para la rama dev más reciente
```
...
KAGE también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/anomalyco/kage/releases) o desde [kage.ai/download](https://kage.ai/download).
...
4. `$HOME/.kage/bin` - Alternativa por defecto
...
KAGE incluye dos agentes integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agentes](https://kage.ai/docs/agents).

### Documentación

Para más información sobre cómo configurar KAGE, [**ve a nuestra documentación**](https://kage.ai/docs).

### Contribuir

Si te interesa contribuir a KAGE, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Proyectos basados en KAGE

Si estás trabajando en un proyecto basado en KAGE y usas "kage" como parte del nombre, por ejemplo, "kage-dashboard" u "kage-mobile", agrega una nota en tu README para aclarar que no está hecho por el equipo de KAGE y que no está afiliado con nosotros de ninguna manera.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/kage) | [X.com](https://x.com/kage)
