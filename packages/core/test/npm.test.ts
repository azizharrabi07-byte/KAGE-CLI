...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@kage/acme")).toBe("@kage/acme")
    expect(Npm.sanitize("@kage/acme@1.0.0")).toBe("@kage/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/kage/acme.git"
    const expected = win ? "acme@git+https_//github.com/kage/acme.git" : spec
