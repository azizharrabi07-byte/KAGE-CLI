---
title: CLI
description: Opciones y comandos de la CLI de KAGE.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de KAGE de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
kage
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con KAGE mediante programación.
...
Inicie la interfaz de usuario del terminal KAGE.
...
La CLI de KAGE también tiene los siguientes comandos.

---

### agent

Administrar agentes para KAGE.
...
Conecte una terminal a un servidor backend KAGE que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
kage attach [url]
```

Esto permite usar TUI con un backend remoto KAGE. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `kage`) |
...
KAGE funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `kage auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Cuando se inicia KAGE, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en KAGE.
...
Ejecute kage en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `kage serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor kage en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `kage`) |
...
Inicie un servidor KAGE sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
kage serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad kage sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `kage`).
...
Enumere todas las sesiones KAGE.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones KAGE.
...
Importe datos de sesión desde un archivo JSON o una URL compartida KAGE.

```bash
kage import <file>
```

Puede importar desde un archivo local o una URL compartida KAGE.
...
Inicie un servidor KAGE sin cabeza con una interfaz web.

```bash
kage web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a KAGE a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `kage`).
...
Desinstale KAGE y elimine todos los archivos relacionados.
...
Actualiza kage a la última versión o a una versión específica.
...
La CLI de KAGE toma las siguientes banderas globales.
...
KAGE se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `kage`) |
