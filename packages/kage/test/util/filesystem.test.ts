...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "kage.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "kage.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kage.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kage.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["kage.json", "kage.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "kage.json"),
        path.join(project, "kage.jsonc"),
      ])
    })
  })
