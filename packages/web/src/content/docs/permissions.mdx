...
KAGE uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start KAGE with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
kage --auto
```

You can also use auto mode with [`kage run`](/docs/cli#run).

```bash
kage run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where KAGE was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
KAGE permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, KAGE starts from permissive defaults:
...
When KAGE prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current KAGE session)
