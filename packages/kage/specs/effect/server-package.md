...
Practical reference for a future `packages/server` split after the kage
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/kage`.
...
Until enough shared service code lives outside `packages/kage`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/kage`

It should not import `packages/kage` services while `packages/kage`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on kage-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/kage` implementation details.
