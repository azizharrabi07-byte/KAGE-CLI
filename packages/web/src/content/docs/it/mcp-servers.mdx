...
Puoi aggiungere strumenti esterni a KAGE usando il _Model Context Protocol_, o MCP. KAGE supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [KAGE Config](https://kage.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/kage`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
KAGE gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, KAGE:
...
Se il server richiede autenticazione, KAGE ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `kage mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, KAGE memorizzera' i token in modo sicuro in `~/.local/share/kage/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in KAGE insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione KAGE come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare KAGE al tuo account Sentry.
