---
title: ACP-støtte
description: Bruk kage i en hvilken som helst ACP-kompatibel editor.
---

KAGE støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke KAGE via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `kage acp`.

Kommandoen starter KAGE som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'KAGE'-agenten i AI Chat-agentvelgeren.
...
For å bruke KAGE som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke KAGE som ACP-agent for chat.
...
KAGE fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i KAGE-konfigurasjonen
