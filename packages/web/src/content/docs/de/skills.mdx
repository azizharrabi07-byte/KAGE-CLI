...
Agent Skills erlauben KAGE, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
KAGE sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft KAGE vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.kage/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/kage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.kage/skills/git-release/SKILL.md` zum Beispiel so:
...
KAGE listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `kage.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `kage.json`):
