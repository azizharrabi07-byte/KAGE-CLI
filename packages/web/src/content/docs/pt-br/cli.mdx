---
title: CLI
description: Opções e comandos da CLI do kage.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do kage, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
kage
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o kage programaticamente.
...
Inicie a interface de usuário do terminal do kage.
...
A CLI do kage também possui os seguintes comandos.

---

### agent

Gerencie agentes para o kage.
...
Anexe um terminal a um servidor backend do kage já em execução, iniciado via comandos `serve` ou `web`.

```bash
kage attach [url]
```

Isso permite usar o TUI com um backend kage remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `kage`) |
...
O kage é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `kage auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/kage/auth.json`.

```bash
kage auth login
```

Quando o kage é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no kage.
...
Execute o kage em modo não interativo passando um prompt diretamente.
...
```bash "kage run"
kage run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `kage serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
kage run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor kage em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `kage`) |
...
Inicie um servidor kage sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
kage serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do kage sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `kage`).
...
Liste todas as sessões do kage.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do kage.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do kage.

```bash
kage import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do kage.
...
Inicie um servidor kage sem cabeça com uma interface web.

```bash
kage web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o kage através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `kage`).
...
Desinstale o kage e remova todos os arquivos relacionados.
...
Atualiza o kage para a versão mais recente ou uma versão específica.
...
A CLI do kage aceita as seguintes flags globais.
...
O kage pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `kage`) |
