...
Per diagnosticare problemi con KAGE, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\kage\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `kage --log-level DEBUG`.

---

## Archiviazione

kage salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/kage/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\kage`
...
KAGE Desktop esegue in background un server locale di KAGE (il sidecar `kage-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `KAGE` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/kage/kage.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\kage\kage.jsonc`
...
KAGE puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/kage/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\kage\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che KAGE possa ricostruirla.

1. Chiudi completamente KAGE Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/kage`
- **Linux**: elimina `~/.cache/kage` (oppure esegui `rm -rf ~/.cache/kage`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\kage`

3. Riavvia KAGE Desktop.

---

### Risolvi problemi di connessione al server

KAGE Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `kage.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, KAGE Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di KAGE.

---

### Notifiche non visualizzate

KAGE Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per KAGE nelle impostazioni del sistema operativo, e
...
1. Chiudi KAGE Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app KAGE Desktop):

- `kage.settings.dat` (desktop default server URL)
- `kage.global.dat` e `kage.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con KAGE:
...
3. Assicurati di avere l'ultima versione con `kage upgrade`
...
Per capire a quali modelli hai accesso, esegui `kage models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\kage`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. kage installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\kage`

2. Riavvia kage per reinstallare i pacchetti provider piu recenti

Questo forzera kage a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
kage rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
