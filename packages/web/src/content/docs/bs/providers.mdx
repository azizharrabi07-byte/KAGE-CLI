---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u KAGE.
---

import config from "../../../../config.mjs"
export const console = config.console

KAGE koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj KAGE konfiguraciji.
...
u `~/.local/share/kage/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem KAGE
config.
...
KAGE Zen je lista modela koje je obezbedio KAGE tim koji su bili
testirano i potvrđeno da dobro radi sa KAGE. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa KAGE Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite kage i idite na [kage.ai/auth](https://kage.ai/auth).
...
Radi kao i svaki drugi provajder u KAGE i potpuno je opcionalan za korištenje.

---

## KAGE Go

KAGE Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža KAGE tim i koji su testirani i verificirani da dobro rade s KAGE-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `KAGE Go` i idite na [kage.ai/auth](https://kage.ai/zen).
...
Da biste koristili Amazon Bedrock s KAGE:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete kage:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `kage.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u KAGE.
:::
...
Možete konfigurirati kage za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi kage ispravno radio.
   :::
...
Također možete dodati modele kroz svoju kage konfiguraciju.
...
da pohrani token u kage auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
KAGE koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate KAGE
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`kage.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth).

##### Konfiguracija

Prilagodite putem `kage.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s kage:
...
Za korištenje Google Vertex AI s KAGE:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće kage.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem kagea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session) da automatski prijavite svaki KAGE razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki KAGE razgovor naveden kao zasebna sesija.
...
Možete konfigurirati kage za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati kage za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati kage za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za KAGE. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/kage) za detalje.
:::
...
Da biste koristili Ollama Cloud s KAGE:
...
3. Kopirajte API ključ za korištenje u KAGE.
...
6. **Važno**: Prije upotrebe modela oblaka u KAGE, morate lokalno povući informacije o modelu:
...
KAGE Zen je lista testiranih i verifikovanih modela koju je obezbedio KAGE tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>KAGE Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **KAGE Zen**.

```txt
   /connect
```

3. Unesite svoj KAGE API ključ.
...
Također možete dodati dodatne modele putem vaše kage konfiguracije.
...
5. Također ih možete prilagoditi putem vaše kage konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju kage konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s kage-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `kage.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju KAGE da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `kage auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite KAGE konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj kage konfiguraciji.
