---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć kage.
---

Wtyczki umożliwiające wyłączenie kage poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania kage.
...
- `.kage/plugins/` - Wtyczki na poziomie projektu
- `~/.config/kage/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/kage/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/kage/kage.json`)
2. Konfiguracja projektu (`kage.json`)
3. Globalny katalog wtyczek (`~/.config/kage/plugins/`)
4. Katalog wtyczek projektu (`.kage/plugins/`)
...
kage uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z kage do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do kage.
...
:::note
Jeśli korzystasz z aplikacji komputerowej kage, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj kage czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do kage:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod kage. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://kage.ai/docs/sdk).
