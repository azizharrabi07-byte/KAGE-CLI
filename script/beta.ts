...
const model = "kage/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/kage")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with kage...`)
  ...
  try {
    await $`kage run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  kage failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with kage")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with kage...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/kage`.",
