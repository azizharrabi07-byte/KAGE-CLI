---
title: Zen
description: Lista selecionada de modelos fornecidos pelo KAGE.
---
...
O KAGE Zen é uma lista de modelos testados e verificados fornecidos pela equipe do KAGE.

O Zen funciona como qualquer outro provedor no KAGE. Você faz login no KAGE Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o KAGE.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o KAGE.
:::
...
O KAGE Zen é um gateway de AI que dá a você acesso a esses modelos.

---

## Como funciona

O KAGE Zen funciona como qualquer outro provedor no KAGE.

1. Você faz login no **<a href={console}>KAGE Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona KAGE Zen e cola sua chave de API.
...
O [model id](/docs/config/#models) na sua configuração do KAGE usa o formato `kage/<model-id>`. Por exemplo, para GPT 5.5, você usaria `kage/gpt-5.5` na sua configuração.
...
Você pode notar [modelos de baixo custo](/docs/config/#models), como Haiku, Nano ou Flash, no seu histórico de uso. O KAGE usa esses modelos para gerar títulos de sessões.
...
- DeepSeek V4 Flash Free está disponível no KAGE por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- MiMo-V2.5 Free está disponível no KAGE por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- North Mini Code Free está disponível no KAGE por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3 Ultra Free está disponível no KAGE por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo stealth que está gratuito no KAGE por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o KAGE também.
