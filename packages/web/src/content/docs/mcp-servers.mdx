...
You can add external tools to KAGE using the _Model Context Protocol_, or MCP. KAGE supports both local and remote servers.
...
You can define MCP servers in your [KAGE Config](https://kage.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/kage` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
KAGE automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, KAGE will:
...
If the server requires authentication, KAGE will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `kage mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, KAGE will store the tokens securely in `~/.local/share/kage/mcp-auth.json`.
...
Your MCPs are available as tools in KAGE, alongside built-in tools. So you can manage them through the KAGE config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect KAGE to your Sentry account.
