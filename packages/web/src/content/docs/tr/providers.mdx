---
title: Sağlayıcılar
description: kage'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

kage, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı kage yapılandırmanızda yapılandırın.
...
`~/.local/share/kage/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları kage'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
KAGE Zen, kage ekibi tarafından sağlanan modellerin bir listesidir.
kage ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz KAGE Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, kage'u seçin ve [kage.ai/auth](https://kage.ai/auth)'ye gidin.
...
kage'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## KAGE Go

KAGE Go, kage ile iyi çalıştığı test edilmiş ve doğrulanmış, kage ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `KAGE Go`'yu seçin ve [kage.ai/auth](https://kage.ai/zen) adresine gidin.
...
Amazon Bedrock'u kage ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   kage'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `kage.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi kage'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
kage'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   kage'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
kage, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. kage'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`kage.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [kage-gitlab-auth](https://www.npmjs.com/package/kage-gitlab-auth) ana sayfasında.

##### Yapılandırma

`kage.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi kage'la kullanmak için:
...
Google Vertex AI'yi kage ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları kage çalıştırılırken ayarlayın.
...
Helicone'un kage aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her kage görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [kage-helicone-session](https://github.com/H2Shami/kage-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her kage görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
kage'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla kage'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla kage'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini kage için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/kage)'a bakın.
:::
...
Ollama Cloud'u kage ile kullanmak için:
...
3. kage'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: kage'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
KAGE Zen, kage ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>KAGE Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **KAGE Zen**'i arayın.

   ```txt
   /connect
   ```

3. kage API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   kage yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca kage yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri kage yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı kage ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `kage.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları kage'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `kage auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için kage yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, kage yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
